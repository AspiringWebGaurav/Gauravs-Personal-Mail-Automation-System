...
interface AuditLog {
    ...
    reminderId?: string;
    errorDetails?: string; // Legacy
    errorMessage?: string; // New
    provider?: string;
    durationMs?: number;
    metadata?: { simulation?: boolean;[key: string]: unknown };
}
                    ...
                    >
                        <div style={{
                            ...
                            {logs.map(log => {
                                ...
                                if (log.status === 'SENT' || log.status === 'SIMULATION_ON') color = '#10b981';
                                if (log.status === 'FAILED' || log.status === 'SIMULATION_OFF') color = '#ef4444';
                                if (log.status === 'HALTED') color = '#fbbf24';
                                if (log.status === 'PENDING') color = '#f59e0b';

                                const isSimulated = log.metadata?.simulation === true;
                                const errMsg = log.errorMessage || log.errorDetails;
                                    ...
                                    <div key={log.id} style={{
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                                ...
                                                {isSimulated && (
                                                    <span style={{ fontSize: '0.6rem', background: '#8b5cf6', color: 'white', padding: '1px 6px', borderRadius: 99 }}>
                                                        SIMULATED
                                                    </span>
                                                )}
                                            </div>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-secondary)' }}>
                                            ...
                                            <div style={{ display: 'flex', gap: 8, fontSize: '0.75rem' }}>
                                                {log.provider && <span>via {log.provider}</span>}
...
function SimulationModeControl() {
    const { isSimulationMode } = useSystemControl();
    const { toggleSimulationMode } = require('@/services/participantServiceFixed'); // Lazy load to avoid circular deps if any
    const [toggling, setToggling] = useState(false);

    const handleToggle = async () => {
        setToggling(true);
        try {
            await toggleSimulationMode(!isSimulationMode);
        } catch (e) {
    ...
    return (
        <motion.div
            className={`card ${styles.simulationCard}`}
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            style={{
                borderColor: isSimulationMode ? '#8b5cf6' : 'var(--border)', // Purple for simulation
                background: isSimulationMode ? 'rgba(139, 92, 246, 0.1)' : 'var(--card-bg)',
                marginBottom: '1rem',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'space-between',
                padding: '1rem',
                borderWidth: isSimulationMode ? '2px' : '1px',
                borderStyle: 'solid'
            }}
        >
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <div style={{
                    background: isSimulationMode ? '#8b5cf6' : 'var(--bg-secondary)',
                    padding: '0.5rem',
                    borderRadius: '8px',
                    color: isSimulationMode ? 'white' : 'var(--text-secondary)'
                }}>
                    <Zap size={24} />
                </div>
                <div>
                    <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 600, color: isSimulationMode ? '#8b5cf6' : 'var(--text-primary)' }}>
                        {isSimulationMode ? 'SIMULATION MODE ACTIVE' : 'System Normal'}
                    </h3>
                    <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                        {isSimulationMode ? 'Emails are MOCKED. No real emails will be sent.' : 'Real emails are enabled.'}
                    </p>
                </div>
            </div>
            <button
                ...
                disabled={toggling}
                style={{
                    background: isSimulationMode ? 'var(--card-bg)' : 'var(--bg-secondary)',
                    color: isSimulationMode ? '#8b5cf6' : 'var(--text-primary)',
                    border: isSimulationMode ? '1px solid #8b5cf6' : '1px solid var(--border)',
                    padding: '0.5rem 1rem',
                    borderRadius: '6px',
                    fontWeight: 600,
            ...
            >
                {toggling ? 'Updating...' : (isSimulationMode ? 'Disable Simulation' : 'Enable Simulation')}
            </button>
        </motion.div>
    );
}
...
export default function SettingsPage() {
    ...
    useEffect(() => {
        ...
        // Listen directly to Firestore (no legacy env providers)
        try {
            ...
            console.warn('[Providers] Subscription init failed:', e);
