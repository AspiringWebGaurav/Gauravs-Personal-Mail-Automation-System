...
export async function createScheduledReminder(data: {
    ...
    try {
        const docId = await runTransaction(db, async (transaction: any) => {
            ...
            const reminderData = {
                ...data,
                senderName: data.userName || 'GPMAS User',
...
export async function toggleSimulationMode(enabled: boolean): Promise<void> {
    const { setDoc } = await import('firebase/firestore');
    await setDoc(doc(db, 'systemSettings', 'globalConfig'), { simulationMode: enabled }, { merge: true });
    trackWrite();

    // AUDIT LOG
    await logMailAction({
        action: 'SIMULATION_TOGGLE',
        status: enabled ? 'SIMULATION_ON' : 'SIMULATION_OFF',
        metadata: { scope: 'global', newValue: enabled }
    });
}
